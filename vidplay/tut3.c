...
int pre_context_init(VPPresContext *presCtx) {
...
void pres_context_close(VPPresContext *presCtx) {
...
int vid_context_init(VPVidContext *vidCtx, const char *path) {
...
int play_video(VPVidContext *vidCtx, VPPresContext *presCtx) {
  AVPacket packet;
  SDL_Event event;
  int frameIdx = 0;
    ...
    if (packet.stream_index == vidCtx->videoStreamIdx) {
      ...
      while (1) {
        ...
        if (recvRet == AVERROR(EAGAIN)) {
          ...
          uint8_t *pixels;
          int pitch;
          if (SDL_LockTexture(presCtx->texture, NULL, (void **)&pixels,
                              &pitch) < 0) {
...
int main(int argc, char *argv[]) {
  ...
  if (pre_context_init(&presCtx) < 0) {
    LOG_ERROR("pre_context_init failed");
    return -1;
  }
